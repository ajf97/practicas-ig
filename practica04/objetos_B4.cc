...
_triangulos3D::_triangulos3D()
{
	...
	ambiente_difusa[0] = 0.7;ambiente_difusa[1] = 0.7;ambiente_difusa[2] = 0.7;ambiente_difusa[3] = 1.0;
    especular[0] = 0.7;especular[1] = 0.7;especular[2] = 0.7;especular[3] = 1.0;
	brillo = 100;
}
...
{
	// Para la iluminación plana necesitamos calcular las normales de los triángulos
	// para aplicar la normal
	...
	// Aplicamos los materiales
	
	glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, (GLfloat *)&ambiente_difusa);
	glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, (GLfloat *)&especular);
	glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, brillo);

	// En este caso al ser iluminación plana, tenemos que aplicar una normal a los 3 vértices que forman la cara.
	// De esta forma se obtiene un color uniforme en toda la cara
...
void _triangulos3D::draw_iluminacion_suave()
{
	...
	// Necesitamos las normales de los vértices para hacer el suavizado Goraud
	...
	// En este caso, tenemos que aplicar una normal a cada uno de los vértices.
	// Por tanto, cada uno de los 3 vértices tienen un color distinto y opengl realiza
	// una interpolación en el resto de vértices, consiguiendo una apariencia más curva.
...
{
	// Si no tenemos las normales de las caras las calculamos.
	...
	// Inicializamos las normales de los vértices a 0
	...
	// Calculamos las normales de los vértices a partir de las normales de los triángulos
	// que confluyen en cada uno de los vértices.
	...
	// Por último tenemos que normalizar las normales

	for (unsigned int i = 0; i < vertices.size(); i++)
		normales_vertices[i].normalize();

	b_normales_vertices = true; // Ya tenemos las normales de los vértices
}

void _triangulos3D::calcular_normales_caras()
{
	normales_caras.resize(caras.size()); // Reasignamos el tamaño del vector de las normales

	for (unsigned int i = 0; i < caras.size(); i++)
	{	
		// Realizamos las operaciones que aparecen en el guión de prácticas para el cálculo.

		auto a = vertices[caras[i]._1] - vertices[caras[i]._0];
		auto b = vertices[caras[i]._2] - vertices[caras[i]._0];
		auto n = a.cross_product(b); // Realizamos el producto vectorial

		// A continuación calculamos el valor medio de cada una de las normales, para usarlas 
		// en el cálculo de las normales de los vértices.

		float medio = sqrt(n.x * n.x + n.y * n.y + n.z * n.z);
		normales_caras[i] = _vertex3f(n.x / medio, n.y / medio, n.z / medio);
	}

	b_normales_caras = true; // Ya tenemos las normales de los triángulos
}

void _triangulos3D::set_materials(_vertex4f ambienteDifusa, _vertex4f Especular, float Brillo){

	// Función que modifica los materiales 
	this->ambiente_difusa = ambienteDifusa;
